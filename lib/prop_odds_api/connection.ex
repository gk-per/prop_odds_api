defmodule PropOddsAPI.Connection do
  @moduledoc """
  Handle Tesla connections for PropOddsAPI.

  Additional middleware can be set in the compile-time or runtime configuration:

      config :tesla, PropOddsAPI.Connection,
        base_url: "http://localhost",
        adapter: Tesla.Adapter.Hackney

  The default base URL can also be set as:

      config :prop_odds_api,
        :base_url, "http://localhost"
  """
  ...
  def middleware(options \\ []) do
    base_url =
      Keyword.get(
        options,
        :base_url,
        Application.get_env(:prop_odds_api, :base_url, @default_base_url)
      )
    ...
    user_agent =
      Keyword.get(
        ...
        Keyword.get(
          tesla_options,
          :user_agent,
          "openapi-generator - PropOddsAPI 0.9 - elixir"
        )
      )
